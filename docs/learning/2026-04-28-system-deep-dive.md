...
> **2026-05-25 보정**: `master` 기준 현재 제품에는 구글 로그인/다기기 동기화가 없고, 관계선은 "가까이 두면 자동 연결" 이 아니라 **Shift 드롭으로 명시 연결**합니다. Supabase OAuth 는 `feature/v2-supabase-auth` 에서 실험 중입니다.
...
LAYOUTNEMO 는 **정돈되지 않은 사고의 흐름 그대로** 캔버스 위에 펼쳐놓는 도구입니다. 결로 나누되 칸막이를 치지 않고, Shift 로 겹쳐 놓아 관계를 명시하고, "시급" 만이 아니라 "생각 중" / "머물러 있음" 같은 사고 상태도 색으로 표현합니다.

### 핵심 철학

1. **사고 상태로서의 업무** — 업무는 "할 것 / 한 것" 의 이분법이 아니라 머릿속에서 다양한 상태를 거칩니다. 시급도(stable / thinking / lingering / urgent) 4단계는 그 중 가장 중요한 것을 뽑은 것.
2. **공간 = 의미의 단서** — 두 블럭이 가까이 있으면 관련 가능성이 높고, 사용자는 Shift 드롭으로 그 관계를 확정합니다.
...
| Miro | 화이트보드 | Miro 는 시각적 자유도 위주. 우리는 사고 상태(시급도) + 명시 연결 + AI 보조가 핵심. |
...
**중요**: 위 다이어그램에서 Supabase 부분은 **현재 동작하지 않습니다**. 오래된 `feature/auth-google` 스파이크와 별개로, 최신 OAuth 실험은 `feature/v2-supabase-auth` 브랜치에 있으며 `master` 에는 아직 머지하지 않았습니다.
...
- 코드 실험은 `feature/v2-supabase-auth` 에 있음.
...
이 외 `/auth/callback/route.ts` 는 `feature/v2-supabase-auth` 브랜치에 있지만 master 에 미머지.
...
**AI 의존도는 의도적으로 낮게 유지.** 핵심 가치(블럭 생성, 결로 분류, Shift 연결, 갈무리, 정리하기)는 AI 끄고도 모두 동작. AI 는 "빠르게 채워주는" 보조일 뿐.
...
`feature/v2-supabase-auth` 브랜치에 코드가 있지만 master 미머지. 활성화되면:
...
이 글은 작성 시점(2026-04-28) 기준이고, 2026-05-25 에 master 기준과 어긋나는 설명을 일부 보정했습니다. v2(Supabase) 작업이 본격 시작되면 큰 부분이 바뀝니다.

**한 줄 요약**: 현재 LAYOUTNEMO 는 클라이언트사이드 단일 페이지 앱 + OpenAI 프록시 두 개. 데이터 저장소는 localStorage. v2 에서 Supabase 로 옮길 계획이지만 아직 안 함. AI 는 도와주는 보조이고 꺼도 모든 핵심 기능 동작.
