...
- **첫 외부 피드백 1차 반영 배포** (9/21, master `b64e499`, 롤백 `release/pre-feedback-round-1`): 휠클릭(가운데 버튼) 드래그 팬, Shift 누른 채 갈무리함 드롭 = 즉시 soft-delete(토스트 + Cmd/Ctrl+Z 복귀), 가이드 블럭 갈무리 가능 + 헤더 ? 버튼(`onRestoreGuides`)으로 화면 중앙 재생성, 시급도 글로우 라이트 up·다크 down. 테스트 `tests/canvas-middle-pan-and-dock-delete.test.tsx`. **사용자가 거절한 제안(다시 꺼내지 말 것)**: 블럭 호버 툴바/신호등 점, 블럭 색·라벨 시급도, 휴지통 독, 연결 블럭 강조(결 강조 유지), 잠금(lock, 홀드), 결/갈무리 네이밍 변경, 헤더 반응형 재작업(가로 스크롤로 충분). 실기기 확인 대기: 글로우 감각, Firefox 중클릭 자동 스크롤, ? 생성 위치.
